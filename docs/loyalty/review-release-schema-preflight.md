...
collection/reminder, shared-delivery and owner-privacy readers. It is a
**necessary check, not migration approval or full compatibility proof**. The
[M1 acceptance packet](store-review-acceptance-packet.md)
...
delivery tables, three owner-privacy tables, the collection and delivery fields
on existing review settings/requests, the shared delivery policy field, the
append-only outbox enum and the reviewed indexes declared for these tables. It
compares all scalar/enum columns of those Prisma models with target metadata and
checks release-critical state/identity
...
prove every default, grants, provider-specific DDL behavior, existing data, transaction
semantics, background jobs or rollback of persisted export phases.

Local verification at the source revision of this document: ten focused
