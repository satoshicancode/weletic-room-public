...
export function createMerchantLedgerRowExportAction(
  ...
  return async (request: Request) => {
    ...
    try {
      ...
      const parsed = merchantLedgerRowExportRequestSchema.safeParse(value);
      ...
      return await authenticate(request, async ({ actor }) => {
        ...
        return reply(
          verifyMerchantLedgerRowExportResponse(parsed.data, data),
          200,
        );
      });
    } catch (error) {
