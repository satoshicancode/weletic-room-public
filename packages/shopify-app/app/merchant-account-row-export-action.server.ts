...
export function createMerchantAccountRowExportAction(
  ...
  return async (request: Request) => {
    ...
    try {
      ...
      const parsed = merchantAccountRowExportRequestSchema.safeParse(value);
      ...
      return await authenticate(request, async ({ actor }) => {
        ...
        return reply(
          verifyMerchantAccountRowExportResponse(parsed.data, data),
          200,
        );
      });
    } catch (error) {
