...
export function createMerchantTierHistoryExportAction(
  ...
  return async (request: Request) => {
    ...
    try {
      ...
      const parsed = merchantTierHistoryExportRequestSchema.safeParse(value);
      ...
      return await authenticate(request, async ({ actor }) => {
        const data = await weleticApiJson<unknown>(
          "/api/internal/shopify/merchant/analytics/tier-history",
          {
            method: "POST",
        ...
        return reply(
          verifyMerchantTierHistoryExportResponse(parsed.data, data),
          200,
        );
      });
    } catch (error) {
