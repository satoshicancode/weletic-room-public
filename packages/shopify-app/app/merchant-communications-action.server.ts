...
export function createMerchantCommunicationsAction(
  ...
  return async (request: Request) => {
    ...
    try {
      ...
      const parsed = loyaltyCommunicationsRequestSchema.safeParse(value);
      ...
      return await authenticate(request, async ({ actor }) => {
        ...
        return reply(
          verifyLoyaltyCommunicationsResponse(parsed.data, result),
          200,
        );
      });
    } catch (error) {
