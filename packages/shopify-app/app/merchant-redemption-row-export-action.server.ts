...
export function createMerchantRedemptionRowExportAction(
  ...
  return async (request: Request) => {
    ...
    try {
      ...
      const parsed = merchantRedemptionRowExportRequestSchema.safeParse(value);
      ...
      return await authenticate(request, async ({ actor }) => {
        ...
        return reply(
          verifyMerchantRedemptionRowExportResponse(parsed.data, data),
          200,
        );
      });
    } catch (error) {
