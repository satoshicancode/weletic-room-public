...
export function createMerchantLoyaltyNudgeAction(
  ...
  return async (request: Request) => {
    ...
    try {
      ...
      const parsed = loyaltyNudgeRequestSchema.safeParse(value);
      ...
      return await authenticate(request, async ({ actor }) => {
        ...
        return reply(verifyLoyaltyNudgeResponse(parsed.data, result), 200);
      });
    } catch (error) {
      const status =
