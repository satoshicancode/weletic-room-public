...
const configuredAppDomain = process.env.NEXT_PUBLIC_APP_DOMAIN?.replace(
  /\/$/,
  "",
);
const configuredApiDomain = process.env.NEXT_PUBLIC_API_DOMAIN?.replace(
  /\/$/,
  "",
);
const configuredPartnersDomain =
  process.env.NEXT_PUBLIC_PARTNERS_DOMAIN?.replace(/\/$/, "");
const configuredAdminDomain = process.env.NEXT_PUBLIC_ADMIN_DOMAIN?.replace(
  /\/$/,
  "",
);

const hostnameFromDomain = (domain?: string) => {
...
export const APP_HOSTNAMES = new Set([
  ...
  "localhost:8888",
  "localhost",
  ...(hostnameFromDomain(configuredAppDomain)
    ? [hostnameFromDomain(configuredAppDomain)!]
    : []),
]);
  ...
  (process.env.NEXT_PUBLIC_VERCEL_ENV === "production"
    ...
    : process.env.NEXT_PUBLIC_VERCEL_ENV === "preview"
      ? process.env.VERCEL_URL
        ...
        : "https://preview.dub.co"
      : process.env.NEXT_PUBLIC_NGROK_URL || "http://app.localhost:8888");
...
export const API_HOSTNAMES = new Set([
  ...
  "api.localhost:8888",
  "api.localhost",
  ...(hostnameFromDomain(configuredApiDomain)
    ? [hostnameFromDomain(configuredApiDomain)!]
    : []),
]);
...
export const ADMIN_HOSTNAMES = new Set([
  ...
  "admin.localhost",
  ...(hostnameFromDomain(configuredAdminDomain)
    ? [hostnameFromDomain(configuredAdminDomain)!]
    : []),
]);
...
export const PARTNERS_HOSTNAMES = new Set([
  ...
  "partners.localhost",
  ...(hostnameFromDomain(configuredPartnersDomain)
    ? [hostnameFromDomain(configuredPartnersDomain)!]
    : []),
]);

export const PARTNERS_DOMAIN =
  configuredPartnersDomain ??
  (process.env.NEXT_PUBLIC_VERCEL_ENV === "production"
...
export const R2_URL =
  process.env.STORAGE_BASE_URL || "https://assets.weletic.com";
