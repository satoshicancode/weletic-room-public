...
export function useKeyboardShortcut(
  ...
  const id = useId();
  const { enabled, priority, modal, sheet } = options;
  const keySignature = JSON.stringify(key);
  const registeredKeys = useMemo<string[]>(() => {
    const parsedKey = JSON.parse(keySignature) as string | string[];
    return Array.isArray(parsedKey) ? parsedKey : [parsedKey];
  }, [keySignature]);
  ...
  const onKeyDown = useCallback(
    (e: KeyboardEvent) => {
      ...
      if (
        ...
        !!existingModalBackdrop !== !!modal ||
        !!existingSheetBackdrop !== !!sheet
      )
        return;
      ...
      if (!registeredKeys.includes(pressedKey)) return;
    ...
    [registeredKeys, listeners, id, callback, enabled, modal, sheet],
  );
  ...
  useEffect(() => {
    setListeners((prev) => [
      ...
      { id, key: registeredKeys, enabled, priority, modal, sheet },
    ]);

    return () =>
      setListeners((prev) => prev.filter((listener) => listener.id !== id));
  }, [id, registeredKeys, enabled, priority, modal, sheet, setListeners]);
}
