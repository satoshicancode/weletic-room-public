...
try {
  ...
  const names = ["WeleticShopifyStaffGrant", "WeleticShopifyMerchantAction"];
  ...
  // Validate both before the first write. Never execute the unfiltered DB diff:
  ...
  for (let index = 0; index < names.length; index++) {
    ...
    stage = "create-isolated-staff-table";
  ...
  console.log(
    JSON.stringify({
      status: "isolated_staff_schema_validated",
  ...
  console.error(
    `Staff schema staging refused or failed at ${stage}; completed CREATE statements: ${created}. MySQL DDL is not atomic. No destructive retry or credential output.`,
  );
  process.exitCode = 1;
} finally {
