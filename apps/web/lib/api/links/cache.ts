...
class LinkCache {
  ...
  /**
   * Removes every cache layer that can continue serving an old short link.
   * Unlike expireMany, this awaits Redis/runtime deletion and invalidates the
   * response-cache tags before the caller proceeds with irreversible cleanup.
   */
  async invalidateMany(links: Pick<LinkProps, "domain" | "key">[]) {
    if (links.length === 0) return;

    const uniqueLinks = Array.from(
      new Map(
        links.map((link) => [
          this._createKey(link),
          { domain: link.domain, key: link.key },
        ]),
      ).values(),
    );
    const pipeline = redisGlobal.pipeline();
    const runtimeInvalidations: Promise<unknown>[] = [];

    for (const link of uniqueLinks) {
      const cacheKey = this._createKey(link);
      linkLRUCache.delete(cacheKey);
      pipeline.del(cacheKey);
      runtimeInvalidations.push(this._invalidateVercelRuntimeCache(cacheKey));
      revalidateTag(`static:${link.domain.toLowerCase()}:${link.key}`);
      revalidateTag(`static:${link.domain.toLowerCase()}`);
    }

    await Promise.all([pipeline.exec(), ...runtimeInvalidations]);
  }
