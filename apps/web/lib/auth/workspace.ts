...
import { workspaceAuthCache } from "./workspace-cache";
      ...
      try {
        ...
        */
        ...
        if (apiKey) {
          ...
          let limit = 0;
          const interval: `${number} s` | `${number} m` = isAnalytics
            ? "1 s"
            : "1 m";
        ...
        // An explicit route/query identifier must remain authoritative. A
        // restricted token also supplies its project ID; preferring that ID
        // here could let a cached project satisfy a request for a different
        // slug instead of returning not_found.
        const identifier = idOrSlug || workspaceId || workspaceSlug;

        // Attempt in-memory LRU cache lookup first (30s TTL)
        if (identifier && session?.user?.id) {
          const cachedWorkspace = workspaceAuthCache.get({
            identifier,
          ...
          if (cachedWorkspace) {
            workspace = apiKey
              ? {
                  ...cachedWorkspace,
                  users: cachedWorkspace.users.map(
                    ({ workspacePreferences, ...u }) => u,
                  ),
                }
              : cachedWorkspace;
          }
        ...
        if (!workspace) {
            ...
            include: {
              users: {
                ...
                select: {
                  ...
                  workspacePreferences: true, // Always fetch for session reuse in cache
                },
              },
            },
          ...
          // Cache validated member workspace resolution
