...
import { mutateCommissions, mutatePartner } from "@/lib/swr/mutate";
  ...
  const { executeAsync, isPending } = useAction(
    attributeReferringPartnerAction,
    {
      onSuccess: async () => {
        await Promise.all(
          [
            mutatePartner(partner.id),
            referredByPartner?.id ? mutatePartner(referredByPartner.id) : null,
            createCommissionsForPastEvents
              ? mutateCommissions(referredByPartner?.id)
              : null,
          ].filter(Boolean),
        );
