import { z } from "zod";
import { merchantAnalyticsFilterSchema } from "./merchant-analytics-contract";

const instant = z.string().datetime({ offset: true });
const count = z.string().regex(/^(?:0|[1-9]\d*)$/);
const DAY_MS = 86_400_000;
export const MAX_REDEMPTION_ROW_EXPORT_ROWS = 2_000;

export const merchantRedemptionRowExportRequestSchema = z
  .object({
    filter: merchantAnalyticsFilterSchema,
    expectedInstallationGeneration: z.string().min(1).max(64),
  })
  .strict()
  .refine(
    ({ filter }) => {
      if (!filter.startAt || !filter.endAt) return false;
      const first = Date.parse(
        `${new Date(filter.startAt).toISOString().slice(0, 10)}T00:00:00Z`,
      );
      const last = Date.parse(
        `${new Date(filter.endAt).toISOString().slice(0, 10)}T00:00:00Z`,
      );
      const days = Math.floor((last - first) / DAY_MS) + 1;
      return days >= 1 && days <= 366;
    },
    {
      message: "A UTC range of at most 366 days is required",
      path: ["filter"],
    },
  );

const redemptionStatus = z.enum([
  "active",
  "provisioning",
  "issued",
  "used",
  "failed",
  "cancelled",
  "expired",
]);
const artifactKind = z.enum(["discount_code", "gift_card", "store_credit"]);

export const merchantRedemptionRowExportResponseSchema = z
  .object({
    status: z.enum(["available", "too_large"]),
    coverage: z.literal("retained_nonredacted_points_redemptions_only"),
    installationGeneration: z.string().min(1).max(64),
    filter: merchantAnalyticsFilterSchema,
    rows: z
      .array(
        z
          .object({
            accountPseudonym: z.string().regex(/^account_[a-f0-9]{32}$/),
            redemptionPseudonym: z.string().regex(/^redemption_[a-f0-9]{32}$/),
            createdAt: instant,
            currentStatus: redemptionStatus,
            artifactKind,
            pointsSpent: count,
            usedAt: instant.nullable(),
          })
          .strict(),
      )
      .max(MAX_REDEMPTION_ROW_EXPORT_ROWS),
  })
  .strict()
  .refine(({ status, rows }) => status === "available" || rows.length === 0);

export type MerchantRedemptionRowExportRequest = z.infer<
  typeof merchantRedemptionRowExportRequestSchema
>;
export type MerchantRedemptionRowExportResponse = z.infer<
  typeof merchantRedemptionRowExportResponseSchema
>;

export function verifyMerchantRedemptionRowExportResponse(
  request: MerchantRedemptionRowExportRequest,
  value: unknown,
) {
  const result = merchantRedemptionRowExportResponseSchema.parse(value);
  if (
    result.installationGeneration !== request.expectedInstallationGeneration ||
    JSON.stringify(result.filter) !== JSON.stringify(request.filter)
  )
    throw new Error("Redemption row export response escaped its request");
  return result;
}
