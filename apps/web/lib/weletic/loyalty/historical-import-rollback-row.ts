  ...
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      if (record.status === "rolled_back") {
        if (!record.reversalLedgerEntryId || !record.rolledBackAt)
          throw new HistoricalImportConflictError();
        return { executionId: record.id, replayed: true, contained: false };
      }
      if (
      ...
      return { executionId: record.id, replayed: false, contained: false };
    }, options);
  } catch (error) {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      if (contained.count !== 1 || stopped.count !== 1)
        throw new HistoricalImportConflictError();
      return { executionId: record.id, replayed: false, contained: true };
    }, options);
  }
}
