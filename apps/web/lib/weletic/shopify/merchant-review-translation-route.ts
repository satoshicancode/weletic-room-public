...
} from "./merchant-review-translations";
import {
...
export function createReviewTranslationRoute(operation: "read" | "write") {
  const schema = z
    .object({
  ...
  return async (request: Request) => {
    try {
      ...
      // Allow the 10,000-character body even with JSON Unicode escaping,
      // while bounding the complete signed actor + input envelope.
      const bytes = await readWeleticShopifyRequestBodyBytes(request, {
        maxBytes: (operation === "write" ? 64 : 16) * 1024,
      });
