...
import { GroupWithProgramSchema } from "./group-with-program";
import {
  base64ImageSchema,
...
export const PartnerSchema = z
  .object({
    ...
    preferredLocale: z.enum(WELETIC_LOCALES).default("en"),
    timeZone: z.string().default("UTC"),
    preferredDisplayCurrency: z
      .string()
      .transform((value): string => normalizeCurrency(value))
      .nullable(),
    preferredPayoutCurrency: z
      .string()
      .transform((value): string => normalizeCurrency(value))
      .nullable(),
    companyName: z
...
export const EnrolledPartnerCompositeSchema =
  EnrolledPartnerSchemaExtended.extend({
    group: GroupWithProgramSchema.nullish(),
    ...
    referral: z
      ...
      .nullish(),
    eligibleBounties: z.array(z.record(z.string(), z.any())).optional(),
    commentsCount: z.number().int().nonnegative().optional(),
    fraudCount: z.number().int().nonnegative().optional(),
  });
