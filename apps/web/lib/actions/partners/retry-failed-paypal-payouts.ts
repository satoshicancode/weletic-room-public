...
export const retryFailedPaypalPayoutsAction = authPartnerActionClient
  ...
  .action(async ({ ctx, parsedInput }) => {
    ...
    try {
      ...
      if (
        !batchResult.successfulPayoutIds.includes(updatedPayout.id) ||
        batchResult.failedPayoutIds.includes(updatedPayout.id)
      ) {
        const errorMsg =
          batchResult.results.find((r) => !r.success)?.error ??
          "PayPal rejected payout batch";
