...
export const createDiscountAction = authActionClient
  ...
  .action(async ({ parsedInput, ctx }) => {
    ...
    const {
      ...
      shopifyDiscountType,
      shopifyConfig: inputShopifyConfig,
      productIds,
      collectionIds,
      bxgy,
      freeShipping,
    ...
    let finalDescription = inputDescription ?? null;
    if (provider === DiscountProvider.shopify) {
      const config = inputShopifyConfig || {
        type: shopifyDiscountType || "amount_off_order",
        productIds: productIds || [],
        collectionIds: collectionIds || [],
        ...(bxgy ? { bxgy } : {}),
        ...(freeShipping ? { freeShipping } : {}),
      };
      finalDescription = JSON.stringify(config);
    }
    ...
    await Promise.all([
      metadataCache.invalidateGroup(programId, groupId, group.slug),
