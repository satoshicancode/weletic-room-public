...
test.describe("Dub Partners onboarding", () => {
  test("complete workspace onboarding with Dub Partners product", async ({
    ...
    const createWsJson = (await createWsRes.json()) as { slug?: string };
    const createdSlug = createWsJson.slug || slug;
    ...
    const billingUpgradePost = page.waitForResponse(
      (r) =>
        r.request().method() === "POST" &&
        r.url().includes(`/api/workspaces/${createdSlug}/billing/upgrade`),
      { timeout: STEP_NAV_TIMEOUT },
    );
    ...
    expect(page.url()).toContain(`/${createdSlug}`);
  });
});
