...
/**
 * Keeps browser reads aligned with direct Prisma billing fixtures. The app
 * server intentionally caches workspace authorization data for 30 seconds;
 * without this route, a test retry can observe the previous billing state.
 */
export async function installWorkspaceBillingReadMock(
  page: Page,
  { slug }: { slug: string },
) {
  await page.route(
    (url) => url.pathname === `/api/workspaces/${slug}`,
    async (route) => {
      ...
      const [response, billingState] = await Promise.all([
        route.fetch(),
      ...
      await route.fulfill({
        ...
        json: {
          ...
          billingCycleEndsAt:
            billingState.billingCycleEndsAt?.toISOString() ?? null,
          subscriptionCanceledAt:
            billingState.subscriptionCanceledAt?.toISOString() ?? null,
        },
      });
    },
  );
