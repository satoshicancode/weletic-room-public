...
import { createMerchantLoyaltyConfigurationAction } from "../../../../packages/shopify-app/app/merchant-loyalty-configuration-action.server";
...
describe("Remix Loyalty configuration signed transport adapter", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  ...
  function setup() {
    ...
    const withActor: Parameters<
      typeof createMerchantLoyaltyConfigurationAction
    ...
    return {
      authenticate,
      handle: createMerchantLoyaltyConfigurationAction(withActor),
    };
  }
  const incoming = (value: unknown) =>
    new Request("https://fixture.invalid/api/merchant/loyalty-configuration", {
      method: "POST",
  ...
  it("rejects GET and malformed JSON without forwarding", async () => {
    ...
    expect(
      (
        await handle(
          new Request(
            "https://fixture.invalid/api/merchant/loyalty-configuration",
          ),
        )
      ).status,
    ).toBe(405);
