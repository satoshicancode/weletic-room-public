...
import { createMerchantLedgerRowExportAction } from "../../../../packages/shopify-app/app/merchant-ledger-row-export-action.server";
...
describe("merchant ledger-rows action", () => {
  it.each([
    [403, "access_denied"],
    [409, "state_changed"],
    [503, "ledger_rows_unavailable"],
    ...
    const result =
      await createMerchantLedgerRowExportAction(authenticate)(request());
  ...
  it("rejects GET and malformed payloads without authentication", async () => {
    ...
    const handle = createMerchantLedgerRowExportAction(authenticate);
    expect(
      (
        await handle(
          new Request("https://app.example/api/merchant/analytics/ledger-rows"),
    ...
    expect(
      (
        await handle(
          new Request(
            "https://app.example/api/merchant/analytics/ledger-rows",
            {
              method: "POST",
