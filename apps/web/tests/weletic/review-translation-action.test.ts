...
it.each(["read", "write"] as const)(
  ...
  async (operation) => {
    ...
    const handler = createMerchantAction(
      authenticate,
      `review-translations-${operation}`,
    );
    const value = operation === "read" ? { reviewId: "review_1" } : input;
...
it.each(["read", "write"] as const)(
  "preserves privacy 404s for %s, without leaking provider details",
  async (operation) => {
    const authenticate = vi.fn<() => Promise<never>>();
    // The authenticated callback/SQL gateway are covered separately; this checks
    // that the adapter does not turn access removal into an ambiguous 503.
    authenticate.mockRejectedValue(
      new WeleticGatewayError("private content", 404),
    );
    const handler = createMerchantAction(
      authenticate,
      `review-translations-${operation}`,
    ...
    const response = await handler(
      request(operation === "read" ? { reviewId: "review_1" } : input),
    );
