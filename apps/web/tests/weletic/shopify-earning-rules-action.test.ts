...
import { createMerchantEarningRulesAction } from "../../../../packages/shopify-app/app/merchant-earning-rules-action.server";
...
describe("Remix earning-rule signed transport adapter", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  ...
  function setup() {
    ...
    const withActor: Parameters<
      typeof createMerchantEarningRulesAction
    ...
    return {
      authenticate,
      handle: createMerchantEarningRulesAction(withActor),
    };
  }
  const incoming = (value: unknown) =>
    new Request("https://fixture.invalid/api/merchant/earning-rules", {
      method: "POST",
  ...
  it("rejects malformed upstream acknowledgements without forwarding private data", async () => {
    const { handle } = setup();
    mocks.api.mockResolvedValue({ privateField: "not-public" });
    const response = await handle(incoming(save));
