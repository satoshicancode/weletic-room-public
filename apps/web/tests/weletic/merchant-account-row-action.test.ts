...
import { createMerchantAccountRowExportAction } from "../../../../packages/shopify-app/app/merchant-account-row-export-action.server";
...
const request = () =>
  new Request("https://app.example/api/merchant/analytics/account-rows", {
    method: "POST",
...
describe("merchant account-rows action", () => {
  it.each([
    [403, "access_denied"],
    [409, "state_changed"],
    [503, "account_rows_unavailable"],
    ...
    const result =
      await createMerchantAccountRowExportAction(authenticate)(request());
  ...
  it("rejects GET and malformed payloads without authentication", async () => {
    ...
    const handle = createMerchantAccountRowExportAction(authenticate);
    expect(
      (
        await handle(
          new Request(
            "https://app.example/api/merchant/analytics/account-rows",
          ),
        )
      ).status,
    ).toBe(405);
