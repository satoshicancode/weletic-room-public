...
const qstashMocks = vi.hoisted(() => {
  ...
  const publishJSON = vi.fn(
    async (request: {
      ...
      const messageId = `msg_test_${++messageSequence}`;
  ...
  return {
    qstash: {
      messages: {
        get: vi.fn(async (messageId: string) => messages.get(messageId)),
      },
      publishJSON,
    },
  };
...
const assertQstashMessage = async (
  ...
  expect(qstashMessage).toBeDefined();
