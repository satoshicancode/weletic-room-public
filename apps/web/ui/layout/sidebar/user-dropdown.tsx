...
export function UserDropdown() {
  ...
  const menuOptions = useMemo(() => {
    ...
    if (subdomain === "partners") {
      options.push({
        label: t("navigation.switchToWorkspace") || "Switch to workspace",
    ...
    if (subdomain === "app") {
      if (workspaceSlug) {
        options.push({
          label: tAdmin("program.programSettings") || "Workspace settings",
          icon: Gear,
    ...
    return options;
  }, [subdomain, partner, workspaceSlug, setOpenPopover, t, tAdmin]);
    ...
    <Popover
      content={
        <div className="flex w-full flex-col space-y-px rounded-md bg-white p-2 sm:min-w-56">
          ...
          <UserOption
            as="button"
            label={t("navigation.logOut") || tAuth("signOut") || "Log out"}
            icon={LogOut}
            onClick={() => {
              signOut({
                callbackUrl: "/login",
