...
function UpgradedModal({
  ...
  const checkoutWorkspaceSyncRef = useRef<Promise<unknown> | null>(null);

  useEffect(() => {
    if (!checkoutSuccess) {
      checkoutWorkspaceSyncRef.current = null;
      setCheckoutWorkspaceSynced(true);
      return;
    }

    let workspaceSync = checkoutWorkspaceSyncRef.current;
    if (!workspaceSync) {
      setCheckoutWorkspaceSynced(false);
      workspaceSync = mutateWorkspaceData();
      checkoutWorkspaceSyncRef.current = workspaceSync;
    }

    let cancelled = false;
    void workspaceSync.finally(() => {
      if (!cancelled) {
        setCheckoutWorkspaceSynced(true);
      }
    });
  ...
  if (!plan || !showUpgradedModal) return null;
