...
function StartPaidPlanModal({
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const upgradedUrl = new URL(window.location.href);
        upgradedUrl.searchParams.set("upgraded", "true");
        upgradedUrl.searchParams.set("plan", plan ?? "pro");
        window.history.replaceState(window.history.state, "", upgradedUrl);
        setShowModal(false);
        await mutate();
      } else {
