...
import { mutatePartnerLinks } from "@/lib/swr/mutate";
...
const AddPartnerLinkModal = ({
  ...
  const onSubmit = async (formData: FormData) => {
    ...
    try {
      ...
      await mutatePartnerLinks(partner.id);
      toast.success("Link created successfully!");
      onSuccess?.(data);
      setShowModal(false);
      if (data?.shortLink) {
        try {
          await copyToClipboard(data.shortLink, { throwOnError: false });
        } catch {
          // Safe fallback: ignore clipboard write errors when window is not focused
        }
      }
    } catch (error) {
      setErrorMessage(
...
export function useAddPartnerLinkModal({
  ...
  }, [showAddPartnerLinkModal, onSuccess, partner]);
