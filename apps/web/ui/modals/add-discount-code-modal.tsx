import { mutateDiscountCodes } from "@/lib/swr/mutate";
...
interface AddDiscountCodeModalProps {
  ...
  activeDiscountCodes?: DiscountCodeProps[];
}
...
const AddDiscountCodeModal = ({
  ...
  const { discountCodes: fetchedDiscountCodes } = useDiscountCodes({
    partnerId: partner.id || null,
    query: { status: "active" },
    enabled: activeDiscountCodes === undefined,
  });
  ...
  const availableLinks = useMemo(() => {
    ...
    const activeCodes =
      ...
      (partner as any).discountCodes?.filter((d: any) => !d.disabledAt) ??
      [];
    return partnerLinks.filter(
      (link) => !activeCodes.some((c) => c.linkId === link.id && !c.disabledAt),
    );
  }, [partner, activeDiscountCodes, fetchedDiscountCodes]);

  const selectedLink = availableLinks.find((link) => link.id === linkId);

  useEffect(() => {
    if (availableLinks.length === 1 && !linkId) {
      setValue("linkId", availableLinks[0].id);
    } else if (linkId && !availableLinks.some((l) => l.id === linkId)) {
      setValue("linkId", availableLinks[0]?.id || "");
    }
  }, [availableLinks, linkId, setValue]);
  ...
  }, [availableLinks, debouncedSearch]);
  ...
  const onSubmit = async (formData: FormData) => {
    await makeRequest("/api/discount-codes", {
      ...
      onSuccess: async (data) => {
        setShowModal(false);
        reset();
        await mutateDiscountCodes(partner.id);
        if (data?.code) {
          try {
            await copyToClipboard(data.code, { throwOnError: false });
          } catch {
            // Safe fallback: ignore clipboard focus error
          }
        }
