...
export function PartnerInfoCards({
  ...
  const compositeGroup =
    partner && "group" in partner && partner.group !== undefined
      ? (partner as EnrolledPartnerCompositeProps).group
      : null;
  ...
  const group = compositeGroup ?? fetchedGroup;

  const compositeBounties =
    partner &&
    "eligibleBounties" in partner &&
    partner.eligibleBounties !== undefined
      ? (partner as EnrolledPartnerCompositeProps).eligibleBounties
      : null;

  const { data: fetchedBounties, error: errorBounties } = useSWR<
    BountyListProps[]
  >(
    workspaceId && partner && isEnrolled && !compositeBounties
      ? `/api/bounties?workspaceId=${workspaceId}&partnerId=${partner.id}`
      : null,
    fetcher,
  );

  const bounties = compositeBounties ?? fetchedBounties;
...
function ReferredByPartner({
  ...
  const compositeReferral =
    "referral" in partner && partner.referral !== undefined
      ? partner.referral
      : undefined;
  ...
  const referral =
    compositeReferral !== undefined ? compositeReferral : fetchedReferral;
  ...
  if (error && compositeReferral === undefined) {
    return null;
  }

  if (loading && compositeReferral === undefined) {
    return (
