...
function MetadataConditionOperatorMenu({
  ...
  const renderItem = (operator: (typeof CONDITION_OPERATORS)[number]) => (
    <Command.Item
      key={operator}
      onSelect={() => {
        onSelect?.(operator);
  ...
  return (
    <div className="min-w-44 py-1">
      <Command loop tabIndex={-1} className="focus:outline-none">
        <Command.List className="scrollbar-hide flex max-h-64 flex-col gap-0.5 overflow-y-auto">
          <div className="text-content-subtle mx-2 pb-1 pt-1.5 text-xs font-medium">
            Text fields
          </div>
