...
import { Button, ChevronRight, Table, Tooltip, useTable } from "@dub/ui";
...
export function RewardDiscountPartnersCard({
  groupId,
  label = "To",
  emptyLabel = "No partners selected",
  suffix,
  showPreviewAvatars = true,
}: {
  groupId: string;
  label?: string;
  emptyLabel?: string;
  ...
  return (
    <div className="border-border-subtle rounded-xl border bg-white text-sm shadow-sm">
      <button
        ...
        aria-label={`${isExpanded ? "Collapse" : "Expand"} partner recipients`}
        className={cn(
              ...
              {partnersCount === 0 ? (
                ...
                <>
                  ...
                  <PartnerPreviewOrCount
                    ...
                    showPreviewAvatars={showPreviewAvatars}
                  />
                  {suffix ? ` ${suffix}` : null}
                </>
              )}
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Tooltip content={row.original.email}>
            <div
              tabIndex={0}
              className="truncate rounded-sm text-sm text-neutral-600 outline-none focus-visible:ring-2 focus-visible:ring-neutral-400"
            >
              {row.original.email}
            </div>
          </Tooltip>
...
function PartnerPreviewOrCount({
  ...
  const showAvatars = showPreviewAvatars && !isExpanded && partnersCount > 0;
