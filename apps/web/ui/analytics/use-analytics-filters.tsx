...
export function useAnalyticsFilters({
  ...
  const { selectedTab, saleUnit } = context ?? analyticsContext;
  ...
  const LinkFilterItem = useMemo(
    () => ({
      key: "linkId",
      icon: Hyperlink,
      label: "Link",
      getOptionIcon: (_value: any, props: any) => {
    ...
    [links, getFilterOptionTotal],
  );

  const DomainFilterItem = useMemo(
    () => ({
      key: "domain",
      icon: Globe2,
      label: "Domain",
      getOptionIcon: (value: any) => (
        <BlurImage
    ...
    [domains, getFilterOptionTotal],
  );

  const SaleTypeFilterItem = useMemo(
    () => ({
      key: "saleType",
      icon: Receipt2,
      label: "Sale type",
  ...
  const onSelect = useCallback(
    ...
    [queryParams, searchParamsObj, parseFilterParam, filters],
  );
