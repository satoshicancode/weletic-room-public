...
function ClickContent({ event }: { event: any }) {
...
function LeadContent({ event }: { event: any }) {
  return (
...
function SaleContent({ event }: { event: any }) {
  return (
    <div className="flex flex-col gap-1">
  ...
  return isLoading ? (
    ...
    <ul className="flex flex-col gap-5 text-sm">
      {activity.events.map((event, index, events) => {
        ...
        const { icon: Icon, content: Content } = activityData[event.event];
