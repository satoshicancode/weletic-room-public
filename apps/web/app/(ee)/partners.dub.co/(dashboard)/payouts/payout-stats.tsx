...
import { Button, Tooltip, useTranslations } from "@dub/ui";
          ...
          {!isLoading ? (
            <div className="flex items-center gap-2">
              ...
              {amount > 0 && (
                <Button
                  variant="secondary"
                  text={t("payouts.payOutNow") || "Pay out now"}
...
export function PayoutStats() {
  ...
  const payoutStats = [
    {
      label: tCommon("status.pending") || "Pending",
    ...
    {
      label: tCommon("status.processing") || "Processing",
    ...
    ...(["stablecoin", "connect"].includes(partner?.defaultPayoutMethod ?? "")
      ? [
          {
            label: tCommon("status.processed") || "Processed",
          ...
          {
            label: tCommon("status.sent") || "Sent",
