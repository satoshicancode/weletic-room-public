...
export function PartnerProgramOverviewPageClient() {
  ...
  const { programEnrollment, showDetailedAnalytics, loading, error } =
  ...
  if (!program) {
    return (
      <PageWidthWrapper className="pb-10 pt-6">
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-neutral-300 p-12 text-center">
          <p className="text-base font-semibold text-neutral-900">
            Program details not available
          </p>
          <p className="mt-1 text-sm text-neutral-500">
            Please make sure your partner enrollment is active for this program.
          </p>
          <Link
            href="/programs"
            className="mt-4 inline-flex items-center justify-center rounded-lg bg-black px-4 py-2 text-sm font-medium text-white hover:bg-neutral-800"
          >
            Explore Programs
          </Link>
        </div>
          ...
          <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
            ...
            <NumberFlowGroup>
              {showDetailedAnalytics ? (
                <>
                  <StatCard title={t("metrics.totalClicks")} event="clicks" />
                  <StatCard title={t("metrics.totalLeads")} event="leads" />
                  <StatCard title={t("metrics.totalSales")} event="sales" />
                </>
              ) : (
                <>
                  <StatCardSimple
            ...
            ? [
                {
                  ...
                  text: (
                    <div className="flex flex-wrap items-center gap-2">
                      <span>{formatDiscountDescription(discount)}</span>
                      {defaultProgramLink?.discountCode && (
                        <DiscountCodeBadge
                          code={defaultProgramLink.discountCode.code}
                          disabledAt={
                            defaultProgramLink.discountCode.disabledAt
                          }
                        />
                      )}
                    </div>
          ...
          <div className="flex items-center justify-between rounded-t-xl border-t border-neutral-200 bg-white px-4 py-3">
            ...
            {programSlug && (
              <Link href={`/messages/${programSlug}`}>
                <Button
                  variant="secondary"
                  text={t("program.contactProgram") || "Contact program"}
...
function OverviewLoadingSkeleton() {
  return (
    <div className="flex animate-pulse flex-col gap-5 pt-3">
      {/* Rewards Bar Skeleton */}
      <div className="h-28 w-full rounded-xl border border-neutral-200/60 bg-neutral-100/80 p-4" />

      {/* Main Grid: Chart + Payouts */}
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
        <div className="flex h-80 flex-col justify-between rounded-xl border border-neutral-200/80 bg-neutral-50/50 p-5 lg:col-span-2">
          <div className="flex items-center justify-between">
            <div className="h-6 w-32 rounded bg-neutral-200/70" />
            <div className="h-8 w-24 rounded-lg bg-neutral-200/70" />
          </div>
          <div className="h-44 w-full rounded bg-neutral-200/40" />
        </div>
        <div className="flex h-80 flex-col justify-between rounded-xl border border-neutral-200/80 bg-neutral-50/50 p-5">
          <div className="h-6 w-28 rounded bg-neutral-200/70" />
          <div className="h-32 w-full rounded bg-neutral-200/40" />
          <div className="h-9 w-full rounded-lg bg-neutral-200/70" />
        </div>
      </div>

      {/* 3 Metric Cards Skeletons */}
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-3">
        <div className="h-32 rounded-xl border border-neutral-200/80 bg-neutral-50/50 p-5" />
        <div className="h-32 rounded-xl border border-neutral-200/80 bg-neutral-50/50 p-5" />
        <div className="h-32 rounded-xl border border-neutral-200/80 bg-neutral-50/50 p-5" />
      </div>
    </div>
