...
export function EarningsTablePartner({ limit }: { limit?: number }) {
  ...
  const { table, ...tableProps } = useTable({
    ...
    columns: [
      {
        id: "createdAt",
        header: tCommon("status.date") || "Date",
      ...
      {
        id: "type",
        header: tCommon("status.type") || "Type",
        accessorKey: "type",
        meta: {
      ...
      {
        id: "customer",
        header: t("customers.title") || "Customer",
        minSize: 250,
        cell: ({ row }) =>
      ...
      ...(programEnrollment?.rewards?.some((r) => r.event === "sale")
        ? [
            {
              id: "amount",
              header: t("metrics.saleAmount") || "Sale Amount",
              accessorKey: "amount",
              cell: ({ row }) =>
                row.original.amount
        ...
        : [
            {
              id: "country",
              header: tCommon("status.country") || "Country",
      ...
      {
        id: "earnings",
        header: t("metrics.totalEarnings") || "Earnings",
      ...
      {
        header: tCommon("status.title") || "Status",
        cell: ({ row }) => {
    ...
    emptyState: (
      <AnimatedEmptyState
        title={t("commissions.noCommissions") || "No earnings found"}
        description={
          t("commissions.noCommissionsDescription") ||
          "No earnings have been made for this program yet."
        }
        cardContent={() => (
          <>
