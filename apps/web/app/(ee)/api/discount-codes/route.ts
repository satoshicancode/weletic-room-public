...
export const POST = withWorkspace(
  async ({ workspace, req, session }) => {
    ...
    // Check for duplicate active code in the program
    if (code) {
      ...
      if (duplicateByCode && !duplicateByCode.disabledAt) {
        throw new DubApiError({
          code: "conflict",
    ...
    // A link can have only one active discount code
    const duplicateByLink = await prisma.discountCode.findFirst({
      where: {
        linkId,
        disabledAt: null,
      },
    ...
    if (duplicateByLink) {
      throw new DubApiError({
        code: "conflict",
        message:
          "A discount code has already been created for this partner link. Please add a new link to create another code.",
      });
    }
