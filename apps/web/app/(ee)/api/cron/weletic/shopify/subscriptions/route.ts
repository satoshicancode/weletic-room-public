...
import { weleticShopifySubscriptionSweepJob } from "@/lib/jobs/handlers/weletic-shopify-subscription-sweep-job";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const dispatched = await weleticShopifySubscriptionSweepJob.dispatch(
      { scheduledAt, appId },
    ...
    return Response.json(
      { error: "Shopify subscription dispatch unavailable" },
      { status: 503, headers },
    );
  }
}
