...
import { processShopifyComplianceBatch } from "@/lib/weletic/shopify/compliance-worker";
...
export const GET = withCron(async () => {
  ...
  try {
    try {
      const complianceRecovery = await processShopifyComplianceBatch({
        batchSize: 3,
        workerId: `queue_retry_compliance_${crypto.randomUUID()}`,
      });
      logger.info("shopify.compliance_recovery_sweep", {
        selected: complianceRecovery.selected,
        processed: complianceRecovery.processed,
        budgetExhausted: complianceRecovery.budgetExhausted,
      });
    } catch (error) {
      // Compliance rows are durable and this minute-scheduled route is their
      // authoritative recovery sweep. Isolate a failed sweep so generic job
      // replay still runs; the next invocation retries both responsibilities.
      logger.error("shopify.compliance_recovery_sweep_failed", {
