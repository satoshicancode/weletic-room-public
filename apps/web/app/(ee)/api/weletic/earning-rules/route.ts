...
import { manageWorkspaceEarningRules } from "@/lib/weletic/loyalty/workspace-earning-rules";
...
function serviceError(error: unknown) {
  ...
  if (error instanceof EarningRuleWriteError)
    return failure(error.code, error.code === "not_found" ? 404 : 409);
  if (
  ...
  async ({ workspace, permissions }) => {
    try {
      return merchantSettingsJson(
        await manageWorkspaceEarningRules(
          {
...
export const POST = withWorkspace(
  async ({ workspace, permissions, req }) => {
    ...
    const parsed = shopifyEarningRulesInputSchema.safeParse(value);
