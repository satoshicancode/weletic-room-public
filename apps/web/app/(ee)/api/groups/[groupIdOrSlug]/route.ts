...
export const PATCH = withWorkspace(
  async ({ req, params, workspace, session }) => {
    ...
    await metadataCache.invalidateGroup(programId, group.id, group.slug, slug);
    if (
      updateAutoApprovePartnersForAllGroups ||
      updateHoldingPeriodDaysForAllGroups
    ) {
      await metadataCache.invalidateAllGroups(programId);
    }
...
export const DELETE = withWorkspace(
  async ({ params, workspace, session }) => {
    ...
    await Promise.all([
      ...
      metadataCache.invalidateReward(programId),
      metadataCache.invalidateAllGroups(programId),
    ]);
