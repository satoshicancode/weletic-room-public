...
type FormData = Pick<ProgramProps, "minPayoutAmount" | "accountingCurrency">;
...
function ProgramPayoutSettingsSheetContent({
  ...
  useEffect(() => {
    if (program) {
      ...
      setValue("accountingCurrency", program.accountingCurrency);
    }
  }, [program, setValue]);
  ...
  const accountingCurrency = watch("accountingCurrency") ?? "USD";
        ...
        <div className="space-y-3 pb-6">
          <div>
            ...
            <p className="text-sm font-medium text-neutral-500">
              Commission and payout accounting use this currency. It is locked
              after the first financial event.
            </p>
          </div>
          <select
            {...register("accountingCurrency", { required: true })}
            className="h-10 w-full rounded-lg border border-neutral-200 bg-white px-3 text-sm"
          >
            {ACCOUNTING_CURRENCIES.map((currency) => (
          ...
          <div>
            ...
            <NumberFlow
              value={
                minPayoutAmount
                  ? minPayoutAmount /
                    10 ** currencyMinorUnits(accountingCurrency)
                  : 0
              }
              suffix={` ${accountingCurrency}`}
              format={
                {
                  style: "currency",
