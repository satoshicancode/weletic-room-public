...
export default function ProgramPartnerLayout({
  ...
  const { partner, error: partnerError } =
    usePartner<EnrolledPartnerCompositeProps>({
      partnerId: params.partnerId,
      includeComposite: true,
    });

  useEffect(() => {
    if (!partner || !workspaceId) return;

    // 1. Seed Group cache for useGroup
    if (partner.groupId && partner.group) {
      mutate(
        `/api/groups/${partner.groupId}?workspaceId=${workspaceId}`,
        partner.group,
        { revalidate: false },
      );
      if (partner.group.slug && partner.group.slug !== partner.groupId) {
        mutate(
          `/api/groups/${partner.group.slug}?workspaceId=${workspaceId}`,
          partner.group,
          { revalidate: false },
        );
      }
    ...
    // 2. Seed Discount Codes cache for useDiscountCodes
    if (partner.discountCodes) {
      const activeCodes = partner.discountCodes.filter((d) => !d.disabledAt);
      mutate(
        `/api/discount-codes?workspaceId=${workspaceId}&partnerId=${partner.id}&status=all`,
        partner.discountCodes,
        { revalidate: false },
      );
      mutate(
        `/api/discount-codes?partnerId=${partner.id}&workspaceId=${workspaceId}&status=all`,
        partner.discountCodes,
        { revalidate: false },
      );
      mutate(
        `/api/discount-codes?workspaceId=${workspaceId}&partnerId=${partner.id}`,
        activeCodes,
        { revalidate: false },
      );
      mutate(
        `/api/discount-codes?partnerId=${partner.id}&workspaceId=${workspaceId}`,
        activeCodes,
        { revalidate: false },
      );
    }

    // 3. Seed Referral cache for usePartnerReferral
    if (partner.referral !== undefined) {
      mutate(
        `/api/partners/${partner.id}/referral?workspaceId=${workspaceId}`,
        partner.referral,
        { revalidate: false },
      );
    }

    // 4. Seed Eligible Bounties cache for useSWR in PartnerInfoCards
    ...
    // 5. Seed Comments Count cache for usePartnerCommentsCount in PartnerNav & tabs
    if (partner.commentsCount !== undefined) {
      mutate(
        `/api/partners/${partner.id}/comments/count?workspaceId=${workspaceId}`,
        partner.commentsCount,
        { revalidate: false },
      );
    }

    // 6. Seed Fraud Count cache for useFraudGroupCount in PartnerRiskIndicator & PartnerRiskBanner
    ...
    // 7. Seed standard non-composite partner cache key for backward compatibility
    mutate(`/api/partners/${partner.id}?workspaceId=${workspaceId}`, partner, {
      revalidate: false,
    });
  }, [partner, workspaceId, mutate]);
