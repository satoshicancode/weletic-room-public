...
} from "@/lib/weletic/reviews/incentive-merchant-contract";
...
const schema = z
  .object({
    ...
    input: merchantReviewIncentiveActivationInputSchema,
  })
  .strict();
...
export async function POST(request: Request) {
  try {
    ...
    const result = await activateShopifyMerchantReviewIncentive({
      envelope: parsed.data.actor,
      input: parsed.data.input,
    });
    return reply(
      merchantReviewIncentiveActivationResponseSchema.parse(result),
      200,
    );
  } catch (error) {
