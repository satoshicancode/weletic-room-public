...
import { manageShopifyReferralConfigurationInTransaction } from "@/lib/weletic/shopify/referral-configuration";
import {
...
const bodySchema = z
  .object({
    ...
    request: referralConfigurationRequestSchema,
  })
  .strict();
...
export async function POST(request: Request) {
  try {
    ...
    const data = await prisma.$transaction(
      (tx) =>
        manageShopifyReferralConfigurationInTransaction({
          tx,
    ...
    return reply({ error: "referral_configuration_unavailable" }, 503);
  }
}
