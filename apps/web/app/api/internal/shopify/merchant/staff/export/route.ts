...
import {
  exportShopifyStaffInTransaction,
  ShopifyStaffExportCursorError,
} from "@/lib/weletic/shopify/staff-export";
...
const schema = z
  .object({
    ...
    input: shopifyStaffExportInputSchema,
  })
  .strict();
...
/** Owner-only staff-export gateway. Service authentication alone grants nothing:
 * the entire actor + export input is signed, then owner evidence, installation,
 * nonce and permissions are resolved in the export transaction.
 */
export async function POST(request: Request) {
  try {
    ...
    if (error instanceof ShopifyStaffExportCursorError)
