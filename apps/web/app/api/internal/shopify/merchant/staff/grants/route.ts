...
import {
  ...
  ShopifyStaffGrantConflictError,
} from "@/lib/weletic/shopify/staff-grants";
...
const schema = z
  .object({
    ...
    input: replaceShopifyStaffGrantSchema,
  })
  .strict();
...
/** Dedicated merchant gateway. Service authentication alone grants nothing:
 * the entire actor + mutation is signed, then owner evidence, installation,
 * nonce and grant revision are resolved in the mutation transaction.
 */
export async function POST(request: Request) {
  try {
    ...
    return response({ grant: result }, 200);
  } catch (error) {
    ...
    if (
      error instanceof ShopifyStaffGrantConflictError ||
