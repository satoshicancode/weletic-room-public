...
import { rewardCatalogRequestSchema } from "@/lib/weletic/loyalty/reward-catalog-contract";
import { RewardDefinitionConflictError } from "@/lib/weletic/loyalty/rewards";
import { manageShopifyRewardCatalogInTransaction } from "@/lib/weletic/shopify/reward-catalog";
import {
...
const bodySchema = z
  .object({
    ...
    request: rewardCatalogRequestSchema,
  })
  .strict();
...
export async function POST(request: Request) {
  try {
    ...
    const data = await prisma.$transaction(
      (tx) =>
        manageShopifyRewardCatalogInTransaction({
          tx,
    ...
    return reply({ error: "reward_catalog_unavailable" }, 503);
  }
}
